...
def test_cron_parse_simple():
    assert CronExpression.parse("1 2 3 4 5").format() == "1 2 3 4 5"


def test_cron_parse_wildcards():
    assert CronExpression.parse("*/30 1,2 4-6 * *").format() == "*/30 1,2 4-6 * *"


def test_cron_parse_at_macro():
    expression = CronExpression.parse("@daily")
    assert expression.text == "@daily"
    assert expression.format() == "0 0 * * *"
